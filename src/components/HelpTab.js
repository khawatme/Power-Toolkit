    ...
    _getHelpContent() {
        return {
            ...
            automation: {
                ...
                content: 'This tab reveals the automated logic on a table.<br><br><strong>Business Rules:</strong> Shows all rules (both active and inactive) for any table you select. You can <strong>Activate, Deactivate, Open, and Delete</strong> rules directly, and click on any rule to expand it and see its underlying JavaScript logic with syntax highlighting.<br><br><strong>Form Event Handlers:</strong> Lists all <code>OnLoad</code>, <code>OnSave</code>, and <code>OnChange</code> functions configured in the form designer across all main forms. Each handler displays a <strong>Managed</strong> or <strong>Custom</strong> badge. Handlers are sourced from both <code>formxml</code> and <code>formjson</code> definitions.<br><br><strong>Web Resource Editor:</strong> Click the edit button next to any handler to open the associated JavaScript web resource in a built-in code editor. For unmanaged (editable) web resources you can:<br>• Edit the JavaScript code directly in the editor<br>• Upload a new <code>.js</code> file via the drag-and-drop upload zone or the file picker<br>• <strong>Save</strong> — persist the changes without publishing (useful during iterative development)<br>• <strong>Save &amp; Publish</strong> — save and immediately publish the web resource so changes take effect<br><br>Managed or hidden web resources open in a read-only view. A lock banner is shown to indicate the resource cannot be modified.<br><br><strong>💡 Tip:</strong> Use <strong>Save &amp; Publish</strong> to make your script changes live instantly without leaving the toolkit.'
            },
            powerAutomateFlows: {
                title: 'Power Automate Flows',
                summary: 'View, manage, edit, and control Power Automate cloud flows without leaving the toolkit.',
                content: 'This tab displays all solution-aware <strong>Power Automate cloud flows</strong> in your Dataverse environment. For each flow, you can see its name, owner, status (On/Off/Suspended), creation and modification dates.<br><br><strong>Turn On/Off:</strong> Toggle a flow\'s state directly from the card. Managed flows cannot be modified.<br><strong>View / Edit Definition:</strong> Opens the flow definition in a dialog with two views:<br>• <strong>Visual</strong> — a flowchart showing triggers, actions, conditions, loops, and branches in a structured diagram<br>• <strong>JSON</strong> — the raw flow definition with syntax highlighting and copy support. For unmanaged flows, the JSON is fully <strong>editable</strong>. Make your changes and click <strong>Save Definition</strong> to persist them directly to Dataverse. An <strong>Undo</strong> button reverts unsaved edits. Managed flows open in read-only mode with a warning banner.<br><strong>Open in Power Automate:</strong> Opens the flow in the Power Automate maker portal.<br><strong>Delete:</strong> Permanently delete an unmanaged flow (with confirmation).<br><br><strong>💡 Tips:</strong><br>• Use the search bar to filter flows by name, status, or owner.<br>• Edit the JSON definition to quickly fix a misconfigured step or update a connection reference without opening the full Power Automate designer.'
            },
            impersonate: {
                title: 'Impersonate',
